...
int main()
{
	...
	// 넌블로킹 소켓 프로그래밍 : 함수를 일단 반환시키고, 나중에 반환값을 받을 수 있다.
	// => WSAEWOULDBLOCK이 안나올 때까지 반복

	// WSAEvent기반 소켓 프로그래밍 : 이벤트 객체를 만들고, 이를 운영체제한테 전달해서, 네트워크 이벤트가 발생했을 때 감지해서 처리
	// => 최대 64개의 이벤트 객체만 관리하다.
